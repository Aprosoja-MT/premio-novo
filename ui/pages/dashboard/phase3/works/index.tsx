...
import {
  ...
  usePhase3Controller,
  type Phase3ScoreValues,
  type Phase3WorkRow,
} from './usePhase3Controller';
...
export function Phase3WorksPage() {
    ...
    <div className="flex flex-col gap-6">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="mt-1 text-[13px] font-sans text-aprosoja-teal/50">
            {total} obra{total !== 1 ? 's' : ''} finalista{total !== 1 ? 's' : ''}
          </p>
        </div>
      </div>
      ...
      {works.length === 0 ? (
        <div className="flex items-center justify-center py-16 text-[13px] font-sans text-aprosoja-teal/40">
          Nenhuma obra finalista encontrada com esses filtros.
        </div>
      ) : (
...
function ScoreSheet({
  ...
  work: Phase3WorkRow;
  scores: Phase3ScoreValues;
  setScore: (key: keyof Phase3ScoreValues, value: number) => void;
  onSubmit: () => void;
  allScored: boolean;
